...
func (d *DatabaseSqlc) DeleteCalendarEvent(id int) error {
	if d == nil {
...
func (d *DatabaseSqlc) QueryCalendarEventsForMonth(calendarId int, year int, month time.Month, includeEnds bool) (calendar.EventMap, error) {
	if d == nil {
...
func (d *DatabaseSqlc) UpsertCalendarEvent(newCalendarEvent calendar.CalendarEvent) (*CalendarEvent, error) {
	if d == nil {
		return nil, fmt.Errorf("database not initialized")
	}
	calendarEvent, err := d.Queries.GetCalendarEvent(context.Background(), newCalendarEvent.ID)
	if err == nil {
		...
		calendarEvent, err = d.Queries.UpdateCalendarEvent(
			context.Background(),
		...
		calendarEvent, err = d.Queries.CreateCalendarEvent(
			context.Background(),
