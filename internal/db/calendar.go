...
func (d *DatabaseSqlc) UpsertCalendar(newCalendar Calendar) (*Calendar, error) {
	if d == nil {
		return nil, fmt.Errorf("database not initialized")
	}
	calendar, err := d.Queries.GetCalendar(context.Background(), newCalendar.ID)
	if err == nil {
		// Calendar exists, update it
		calendar, err = d.Queries.UpdateCalendar(
			context.Background(),
		...
		calendar, err = d.Queries.CreateCalendar(
			context.Background(),
			newCalendar.Name,
		)
		if err != nil {
