...
func Cmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "smb",
		Short: "Manage Quark's SMB (Samba) network share",
		Long:  "Configure and manage Quark as an SMB/CIFS network share for access from macOS Finder, Windows Explorer, and Linux file managers on the local network.",
	}
...
func setupCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "setup",
		Short: "Install and configure the Quark SMB share (Linux only, requires sudo)",
		Long: `Installs Samba if needed, appends the Quark share block to /etc/samba/smb.conf,
sets the Samba password for a system user, and starts smbd.

Run with sudo: sudo quark smb setup --user <username> --password <password>`,
