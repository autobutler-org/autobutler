...
test.describe('Device Management', () => {
    ...
    test('device manager component loads with clear description', async ({
        page,
    }) => {
        await page.goto('/settings')
        ...
        await expect(deviceManager).toBeVisible({ timeout: 500 })
    ...
    test('enabled devices show data directory path', async ({ page }) => {
        ...
        const enabledBadge = page
            .locator('.device-manager-badge--enabled')
            .first()
    ...
    test('device statuses API endpoint works', async ({ page }) => {
        ...
        if (data.devices.length > 0) {
            ...
            if (device.is_enabled) {
                ...
                expect(device.data_dir).toMatch(
                    /(Autobutler\/data|autobutler\/data)/
                )
    ...
    test('component reloads on refresh-devices event', async ({ page }) => {
        ...
        await expect(deviceManagerContent).toHaveAttribute(
            'hx-trigger',
            /refresh-devices/
        )
    })

    test('device manager is accessible from settings sidebar', async ({
        page,
    }) => {
        await page.goto('/settings')
